...
# record from mic or open the song (located in "test" folder)
# calculate mfcc, quantize if intput in8 used, run inference
#
# usage:
#  python use_model.py --mode [song_name]
...
interpreter = tf.lite.Interpreter(model_path="../models/music_genre.tflite")
...
def run_inference(mfcc_18xT):
    ...
    # If last layer was softmax (it is in your model), y already sums to ~1.
    # Keep a safety softmax if needed:
...
def classify_from_mic(mode):
    if not mode:
        ...
        filepath = os.path.join("../test", mode)
