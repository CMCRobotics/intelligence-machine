#-----------------------------------------------------------------------------------------------------------------------
# train the model - comparison CNN vs LSTM
...
# Split 1 (60% vs 40%)
x_train, x0, y_train, y0 = train_test_split(x, y, test_size=0.40, random_state=1, stratify=y)
# Split 2 (50% vs 50%)
x_test, x_validate, y_test, y_validate = train_test_split(x0, y0, test_size=0.50, random_state=3, stratify=y0)
...
# model.add(Dense(len(LIST_GENRES), activation='softmax'))
...
# model.save('../models/music_genre_cnn.keras') # i save it here but there is some issue to load such model from file to convert to tflite
model.save('../models/music_genre_lstm.keras') # i save it here but there is some issue to load such model from file to convert to tflite
...
# with open("../models/music_genre_cnn.tflite", "wb") as f:
with open("../models/music_genre_lstm.tflite", "wb") as f:
...
# -------------------------------------------------
# Evaluate TFLite model: accuracy + confusion matrix
# -------------------------------------------------

y_pred_tflite = []

for i_value in x_test:
    o_pred_f32 = classify(i_value)
    y_pred_tflite.append(np.argmax(o_pred_f32))

y_pred_tflite = np.array(y_pred_tflite)

# Accuracy
accuracy = np.mean(y_pred_tflite == y_test)
print("TFLite Accuracy:", accuracy)

# Confusion matrix
cm_tflite = confusion_matrix(y_test, y_pred_tflite)

print("\nTFLite confusion matrix (rows=true, cols=pred):")
print(cm_tflite)
